  ...
  return (
    <div className="sidebar">
      <ul >
        ...
        {(isAuthenticated('viewInboundOrders') || isAuthenticated('viewOutboundOrders')) &&
        <li className={(url === '/orders' || url === '/neworder') ? 'active' : ''}>
          <Link to="/orders" className="d-inline-block">
            <img src={(url === '/orders' || url === '/neworder')  ? OrderSelectedIcon : OrderSelectedIcon} alt="Orders" />
        ...
        <li className={(url === '/inventory' || url === '/newinventory' || url === '/productcategory' || url === '/batchexpired' || url === '/batchnearexpiry/product' || url === '/productoutofstock'|| url === '/addproduct'|| url === '/productlist/all') ? 'active' : ''}>
          <Link to="/inventory" className="d-inline-block">
            <img src={(url === '/inventory' || url === '/newinventory' || url === '/productcategory' || url === '/batchexpired' || url === '/batchnearexpiry/product' || url === '/productoutofstock' || url === '/addproduct' || url === '/productlist/all') ? InventoryIcon : InventoryIcon} alt="Inventory" />
        ...
        <li className={(url === '/dashboard')  ? 'active' : ''}>
          <Link to="/dashboard" className="d-inline-block">
            <img src={(url === '/dashboard')  ? NetworkIcon : NetworkIcon} alt="Shippment" />
            <span className="ml-2">Network</span>
          </Link>
        </li>
        }
        {isAuthenticated('trackAndTrace') &&
