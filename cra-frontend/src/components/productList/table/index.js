         ...
         {props.inventories.length === 0 && <div className="rTableRow pt-2 pb-2 justify-content-center text-muted shadow-none">No records found</div>}
          {props.inventories.map((inventory,index) => (
            <div>
              ...
              <div className="rTableCell ml-5">{inventory.createdAt.length>11?inventory.createdAt.substring(8,10)+"/"+inventory.createdAt.substring(5,7)+"/"+inventory.createdAt.substring(0,4):inventory.createdAt}</div>
              <div className="rTableCell ml-5">{inventory.manufacturingDate ? (inventory.manufacturingDate.length>11?inventory.manufacturingDate.substring(5,7)+"/"+inventory.manufacturingDate.substring(0,4):inventory.manufacturingDate.split('/')[1]+"/"+inventory.manufacturingDate.split('/')[2]) : "NAN"}</div>
              <div className="rTableCell">{inventory.expiryDate ? (inventory.expiryDate.length>11?inventory.expiryDate.substring(5,7)+"/"+inventory.expiryDate.substring(0,4):inventory.expiryDate.split('/')[1]+"/"+inventory.expiryDate.split('/')[2]) : "NAN"}</div>    
